...
- **Graph queries read the firehose directly; scanner is the fallback.** `commit-plan` (and any future file-knowledge query, e.g. `brain file <path>`) treats `files.jsonl` as the materialised File↔Session edge table and `prompts.jsonl` as the per-session prompt log, joining them by `(sessionId, timestampUtc)`. The per-file `IBrainSessionScanner` (which re-parses raw `~/.claude/projects/**/*.jsonl`) is only used when the firehose is absent — measured ~20 000× speedup on a 52-file commit-plan when reading the firehose. Consumers auto-trigger `brain ingest --since 7d` first so the firehose is fresh before the join; `--no-refresh` skips that step and `--force-scan` opts back into the raw-JSONL path for debugging.
...
- The planner's last-edit-author heuristic depends on the `FileOpRow.Op` distinction: `write`, `edit`, `multi-edit`, and `notebook-edit` are edit-class; `read` is not. Anything that future-changes the ingest writer's op vocabulary (e.g. adding a new edit-class tool, renaming `multi-edit` → `multiedit`) silently changes which session is assigned as the file's last-editor. The edit-class set lives in `BrainCommitPlanner.EditTools` and the firehose-side equivalent; keep them in sync.
