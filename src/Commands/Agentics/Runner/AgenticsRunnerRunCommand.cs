...
""", ct);
    ...
    private async Task<Dictionary<string, string>> PrepareGitCredentialsAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            var ghToken = stored?.AccessToken ?? SecretValue.None;
            if (!ghToken.HasValue && _console != null)
            {
                _console.MarkupLine(
            ...
            // Composed in the infrastructure layer: the script body contains the token verbatim,
            // so this command hands over the credential and gets back a path.
            var scriptPath = await GitAskpassScript.WriteAsync(ghToken, pksBinary, ct);
    ...
    private static DevcontainerSpawnOptions BuildSpawnOptions(RunnerJob job, string credentialSocketPath, AgenticsRunnerRegistration registration, SecretValue gitToken)
    {
        ...
        // commit-back target for the in-process/vibecast path — not for cloning.
        // The URL embeds the token, so it is composed by GitCloneUrl and stays a SecretValue: this
        // command passes it to the spawner without ever being able to print it.
        var gitBranch = job.AgentDef?.Branch ?? "main";
        var gitUrl = string.IsNullOrEmpty(job.AgentDef?.Repository)
            ? SecretValue.None
            : GitCloneUrl.ForRepository(job.AgentDef.Repository, gitToken);
