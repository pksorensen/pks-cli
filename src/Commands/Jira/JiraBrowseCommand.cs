...
public class JiraBrowseCommand : Command<JiraBrowseCommand.Settings>
{
    ...
    private static async Task LoadWithSpinnerAsync(int cursor, int scrollOffset, string issueKey, Func<Task> work)
    {
        ...
        var label = $" Loading {issueKey}...";
        // Pad to overwrite any previous content on this line
        var width = Math.Max(Console.WindowWidth > 0 ? Console.WindowWidth : 80, label.Length + 6);
        var task = work();
        var spinIdx = 0;

        while (!task.IsCompleted)
        {
            var text = $"  {spinChars[spinIdx % spinChars.Length]}{label}";
            // Move to row col 1, overwrite in-place (no clear)
            Console.Write($"\x1b[{screenRow};1H\x1b[7m{text.PadRight(width)}\x1b[0m");
            spinIdx++;
