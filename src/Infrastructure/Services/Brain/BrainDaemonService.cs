...
public sealed class BrainDaemonService(IBrainPathResolver paths, IProcessRunner processes) : IBrainDaemonService
{
    ...
    public async Task<DaemonResult> InstallAsync(DaemonOptions options, CancellationToken ct = default)
    {
        ...
        WriteExecutable(plan.ScriptPath, plan.ScriptBody, secret: options.Token is { Length: > 0 });
    ...
    public static string BuildShellScript(string exe, DaemonOptions options, string logPath)
    {
        ...
        if (options.Token is { Length: > 0 } token)
            sb.Append($"export PKS_BRAIN_TOKEN='{token.Replace("'", "'\\''")}'\n\n");
    ...
    public static string BuildWindowsScript(string exe, DaemonOptions options, string logPath)
    {
        ...
        if (options.Token is { Length: > 0 } token)
            sb.Append($"set PKS_BRAIN_TOKEN={token}\r\n");
    ...
    /// <param name="secret">The body carries an upload token, so the file must
    /// not be group- or world-readable — the default umask would leave it 0644.</param>
    private static void WriteExecutable(string path, string body, bool secret = false)
    {
        File.WriteAllText(path, body, new UTF8Encoding(false));
        if (IsWindows) return;
