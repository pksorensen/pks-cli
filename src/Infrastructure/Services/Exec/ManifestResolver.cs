...
public sealed class ManifestResolver : IManifestResolver
{
    ...
    public async Task<ResolvedEnvironment?> ResolveAsync(PksManifest manifest, ManifestResolveOptions options)
    {
        ...
        foreach (var capability in manifest.Capabilities)
        {
            ...
            await ResolveEnvironmentAsync(provider, models, options, resolved);
        }
    ...
    private async Task<PksProviderManifest?> ChooseProviderAsync(PksCapabilityManifest capability, ManifestResolveOptions options)
    {
        ...
        {
            if (await IsAvailableAsync(p.Kind))
            {
                available.Add(p);
            }
        }
        ...
        if (available.Count == 0)
        {
            ...
            _console.MarkupLine("  [dim]register one with [bold]pks foundry init[/], or set GEMINI_API_KEY / OPENAI_BASE_URL.[/]");
            return null;
        }
    ...
    /// account.
    /// </summary>
    private async Task<bool> IsAvailableAsync(string kind) => kind.ToLowerInvariant() switch
    {
        ...
        ResolvedEnvironment into)
    {
        ...
        {
            var (value, secret) = await ExpandAsync(template, provider.Kind, models, options);
        ...
        ManifestResolveOptions options)
    {
        ...
        foreach (var match in PlaceholderRx.Matches(template).Reverse())
        {
            ...
            var (value, isSecret) = await ResolveOneAsync(name, providerKind, models, options);
            secret |= isSecret;
